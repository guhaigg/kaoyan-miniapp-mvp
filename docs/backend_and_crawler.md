...
- `POST /api/v1/admin/catalog-refreshes/announcement-foundation` (requires portal admin login)
...
- Offline announcement foundation assets now live in versioned JSON form under `docs/data/`:
  - `school_catalog_snapshot.json`
  - `department_catalog_snapshot.json`
  - `major_catalog_snapshot.json`
  - `announcement_seed_registry.json`
  These are generated by the offline builder / refresh workflow and are intended to become the shared base for announcement seed resolution and future catalog-aware features.
