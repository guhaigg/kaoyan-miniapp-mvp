...
from app.models import AdjustmentOpportunity, AnnouncementPortalCache, Content, ContentSnapshot, CrawlJob, Department, HistoricalAdjustmentProfile, HistoricalReleaseTimingProfile, MentorEvaluation, RawDatasetArchive, School, SiteSection, Source, WorkflowRun, WorkflowStep, utcnow
...
def test_search_announcements_ignores_active_non_asset_workflow_for_rebuilding(client):
    with SessionLocal() as db:
        school = School(name="Ready University", aliases=[])
        db.add(school)
        db.flush()
        source = Source(
            school_id=school.id,
            name="Ready University Portal",
            source_type="official",
            base_url="https://ready.example.edu.cn",
            config={},
            enabled=1,
        )
        db.add(source)
        ...
        db.add(
            SiteSection(
                ...
                name="Ready Notices",
                section_type="notice",
                section_url="https://ready.example.edu.cn/notices/",
                discovery_category="announcement",
        ...
        run = WorkflowRun(
            workflow_type="file_parse",
            scope_type="school",
            scope_key="ReadyUniversity",
            scope_label="Ready University",
            status="running",
            request_payload={},
            result_payload={},
        )
        ...
        db.add(
            WorkflowStep(
                run_id=run.id,
                step_type="file_parse",
                scope_type="school",
                scope_key="ReadyUniversity",
                status="running",
                available_at=utcnow(),
                input_payload={"content_file_id": "pending-file"},
                result_payload={},
            )
        )
        db.commit()

    response = client.post("/api/v1/search/announcements", json={"school_name": "Ready University"})
    assert response.status_code == 200
    payload = response.json()
    assert payload["total"] == 0
    assert payload["asset_state"] == "ready"
...
def test_run_family_discovery_job_announcement_uses_canonical_seed_registry_and_skips_portal_cache():
    ...
    with SessionLocal() as db:
        ...
        assert job.query["seed_source"] == "canonical_registry"
