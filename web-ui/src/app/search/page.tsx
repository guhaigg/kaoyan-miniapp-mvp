...
function buildEmptyResultMessage(
  ...
  if (schoolLikeKeyword) {
    return "当前主搜索词像学校名，但站内暂未命中这所学校的公告。你可以把学校名填到“院校名”里，或再补一个学院/研招关键词重试。";
  }
  if (schoolName && keyword) {
    return "当前学校和关键词组合没有命中公告。也可能是这所学校的相关公告暂未收录，建议先保留学校名或只搜核心词。";
  }
  if (schoolName) {
    return "当前站内还没有命中这所学校的公告。可以补一个学院或研招关键词再试，或稍后回来刷新。";
  }
  if (keyword) {
...
function buildEmptyStateDetail(
  ...
  return hasLocalFilters
    ...
    : "后端没有返回符合条件的公告结果。也可能这所学校暂未收录，建议补学院/研招关键词，或换学校再试。";
}
