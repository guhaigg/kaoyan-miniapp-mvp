  ...
  if (!loggedIn) {
    return (
      <section className="rounded-[2rem] border border-white/80 bg-white/90 p-8 shadow-[0_20px_60px_rgba(122,147,192,0.16)]">
        ...
        <h2 className="mt-3 text-2xl font-bold text-slate-900">登录后，雷达页会告诉你最近哪几个范围最活跃</h2>
        <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
          真正的雷达不是一个设置面板，而是一张信号看板。先登录，之后学校、院系和栏目级命中会在这里压缩成几张清晰的卡。
        </p>
        <div className="mt-6">
  ...
  if (!canManageScopeTargets) {
    return (
      <section className="rounded-[2rem] border border-white/80 bg-white/90 p-8 shadow-[0_20px_60px_rgba(122,147,192,0.16)]">
        ...
        <h2 className="mt-4 text-2xl font-bold text-slate-900">学校、院系和栏目级雷达会放在这里</h2>
        <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
          当前账号仍然可以先用普通订阅铺出动态流；开通高级会员后，这一页会开始展示活跃范围、近窗命中和重点栏目变化。
        </p>
  ...
  return (
    <section className="space-y-6">
      ...
      {overview?.latest_announcement ? (
        <section className="rounded-[2rem] border border-sky-100 bg-white/90 p-6 shadow-[0_20px_60px_rgba(122,147,192,0.16)]">
      ...
      {activeTargets.length === 0 ? (
        <section className="rounded-[2rem] border border-white/80 bg-white/90 p-8 shadow-[0_20px_60px_rgba(122,147,192,0.16)]">
          ...
          <h3 className="mt-4 text-2xl font-bold text-slate-900">最近窗口里还没有明显活跃的雷达范围</h3>
          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-600">
            这不代表雷达没工作，只是这几天命中比较安静。你可以继续去工作台补新范围，或者等下一次集中更新。
          </p>
        </section>
      ) : (
